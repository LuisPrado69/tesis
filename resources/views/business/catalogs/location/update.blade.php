@permission('edit.location.catalogs')
<div class="modal-content" id="location_update">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">
            <span aria-hidden="true">×</span>
        </button>
        <h4 class="modal-title" id="myModalLabel">
            <i class="fa fa-pencil"></i> {{ trans('location.labels.edit') }}
        </h4>
    </div>
    <div class="clearfix"></div>
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_content">
            <form role="form" action="{{ route('update.edit.location.catalogs', ['id' => $entity->id]) }}"
                  method="post" class="form-horizontal form-label-left" id="location_update_fm">
                @method('PUT')
                @csrf
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <input type="text" name="name" id="name" class="form-control"
                               placeholder="{{ trans('location.placeholders.name') }}" value="{{ $entity->name }}"/>
                    </div>
                </div>
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <input type="number" name="latitude" id="latitude" class="form-control"
                               placeholder="{{ trans('location.placeholders.latitude') }}" value="{{ $entity->latitude }}"/>
                    </div>
                </div>
                <div class="item form-group">
                    ...
                    <div class="col-md-4 col-sm-5 col-xs-12">
                        <input type="number" name="longitude" id="longitude" class="form-control"
                               placeholder="{{ trans('location.placeholders.longitude') }}" value="{{ $entity->longitude }}"/>
                    </div>
                </div>
